...
GLvoid Gemstone::SetPosition()
{
	if (ore == NULL) {
		...
		uniform_int_distribution dis_xz{ 0,2000 };
		while (true) {
			bool flag{ false };
			for (int i = 0; i < 3; ++i) {
...
void Bomb::SetPosition()
{
    if (bomb == NULL) {
        ...
        uniform_int_distribution dis_y{ 20,70 };
        uniform_int_distribution dis_xz{ 0,2000 };
        uniform_int_distribution dis0{ 1,2 };
        for (int i = 0; i < 50; ++i) {
...
GLvoid Bubble::Bubble_Setting(int j)
{
    ...
    uniform_int_distribution dis0{ 100,200 };
    uniform_int_distribution dis1{ 0,5 };
